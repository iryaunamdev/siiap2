<div id="accordion-flush-clases-p" data-accordion="collapse"
    data-active-classes="bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
    data-inactive-classes="text-gray-500 dark:text-gray-400"
    class="mt-4">
    <h3 id="accordion-flush-heading-ingreso-p">
        <div class="flex items-center justify-between w-full pb-2 text-left rtl:text-right text-gray-700 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
            <h3 class="uppercase text-xs font-semibold">Materias impartidas (Propedeútico)
            @if (count($tutor->materias_p))
                <span class="pill-primary-600 ml-4">{{ count($tutor->materias_p) }} materias</span>
            @endif
            </h3>

            <button type="button" data-accordion-target="#accordion-flush-body-clases-p" aria-expanded="false"
    ...
    @if(count($tutor->materias_p))
    <div id="accordion-flush-body-clases-p" class="hidden" aria-labelledby="accordion-flush-heading-clases-p">
        <div class="t-head">
        ...
        <div class="t-body">
            @foreach ($tutor->materias_p->sortByDesc('clase.semestre.nombre') as $c )
    ...
    @else
        <span class="italic text-sm text-gray-400">No hay registros de materias de propedéutico.</span>
    @endif
</div>
