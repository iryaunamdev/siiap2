<div id="accordion-flush-graduados-m" data-accordion="collapse"
    ...
    <h3 id="accordion-flush-heading-ingreso-m">
        <div class="flex items-center justify-between w-full pb-2 text-left rtl:text-right text-gray-700 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
            <h3 class="uppercase text-xs font-semibold">Graduaciones (Maestría)
            @if (count($tutor->graduados_m))
                <span class="pill-primary-600 ml-4">{{ count($tutor->graduados_m) }} Graduaciones</span>
            @endif
            </h3>

            <button type="button" data-accordion-target="#accordion-flush-body-graduados-m" aria-expanded="false"
    ...
    @if(count($tutor->graduados_m))
    <div id="accordion-flush-body-graduados-m" class="hidden" aria-labelledby="accordion-flush-heading-graduados-m">
        <div class="t-body">
            @foreach ($tutor->graduados_m->sortByDesc('graduacion.semestre.nombre') as $g )
